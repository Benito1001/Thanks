...
export default class ChallengeTank {
	constructor(x, y, hp, score, setup) {
		this.collide = true;
		this.x = x;
		this.y = y;
		this.maxHp = hp;
	...
	updateY(entities, ctx, dt) {
		...
		if (this.hp == 0) {
			this.dead = true;
			this.score += this.maxHp*100;
			entities.push(new ChallengeTank(this.x, this.y, this.maxHp+2, this.score, true));
		}
	}
	...
	arenafy(entities, ctx, dt) {
		if (!this.setup) {
			...
			for (var i in entities) {
				if (entities[i].type == "tank") {
					if (entities[i].x < this.x-arenaRad) {
						entities[i].x = this.x-arenaRad;
					}
					else if (entities[i].x > this.x+arenaRad-entities[i].width) {
						entities[i].x = this.x+arenaRad-entities[i].width;
					}
				}
			}
		...
		this.score += 1 * dt;
		if (Math.round(this.score/10) % 510 > 255) {
			this.r -= 1;
		}
		else {
			this.r = Math.round(this.score/10) % 510;
		}
		if (Math.round(this.score/2) % 510 > 255) {
			this.g -= 1;
		}
		else {
			this.g = Math.round(this.score/2) % 510;
		}
		if (Math.round(this.score/20) % 510 > 255) {
			this.b -= 1;
		}
		else {
			this.b = Math.round(this.score/20) % 510;
		}
		ctx.font = '50px serif';
		this.text = "Score: " + Math.round(this.score/10);
